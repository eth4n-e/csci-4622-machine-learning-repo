...
def _compare_node(node1, node2):
    ...
    compare_values = (node1.feature_id == node2.feature_id) and np.isclose(
        node1.threshold, node2.threshold, atol=1e-5
    )
    if not compare_values:
        return False
    return _compare_node(node1.left, node2.left) and _compare_node(
...
class TestDT(unittest.TestCase):
    ...
    def test_reduction(self):
        ...
        outs = 0.22222222
        comment = topic + ins + "\n expected output:  \n" + str(outs)
        obtained = impurity_reduction(labels, left_indices, right_indices)
        comment = comment + "\n obtained: " + str(obtained)
        self.assertTrue(np.allclose(obtained, outs, atol=1e-5), msg=comment)
    ...
    def test_split(self):
        labels = np.array([0, 0, 1, 1, 2, 2])
        features = np.array(
            [[1, 0, 0], [1, 1, 0], [0, 1, 1], [1, 0, 1], [1, 1, 1], [0, 0, 0]]
        )
    ...
    def test_importance(self):
        ...
        expected = [0.1, 0.58, 0.31999999999999995, 0.0]
    ...
    def test_build(self):
        ...
        correct_tree = np.load(
            os.path.join(current_folder, "data/tree_depth3_min2.npy"), allow_pickle=True
        )[0]
        topic = "Testing DecisionTree build with depth 3 and min_samples_split 2"
        expected_tree = "\n".join(
            helpers.node_to_string(correct_tree.tree, features_names)[0]
        )
    ...
    def test_gini(self):
        values = [1, 2, 3, 4, 5]
        expected_result = 0.7999999999999999
        result = gini(values)
